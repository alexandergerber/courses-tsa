...
ex() %>% check_function("AIC", index  = 1) %>% check_argument("object") %>%check_equal()
ex() %>% check_function("AIC", index  = 2) %>% check_argument("object")%>% check_equal()
ex() %>% check_function("AIC", index  = 3) %>% check_argument("object")%>% check_equal()
success_msg("Great!")
```
...
After we found a model it is good practice to run some diagnostic checks. Even if we found the most appropriate ARMA model it is not safe to assume that ARMA models where a good choice in the first place (i.e. it could be that all ARMA models are inappropriate).
